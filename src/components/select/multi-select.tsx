...
import { ComboBox as AriaComboBox, Group as AriaGroup, Input as AriaInput, ListBox as AriaListBox, ComboBoxStateContext, Button as AriaButton } from 'react-aria-components'
...
import { type SelectItemType, selectSizes } from './select-context'
...
interface ComboBoxValueProps extends AriaGroupProps {
  ...
  placeholderIcon?: FC<{ className?: string }> | null
...
const TagCloseButton = ({
  ...
  const sizeStyles = {
    sm: { root: 'p-0.5', icon: 'sm' as const },
    md: { root: 'p-0.5', icon: 'md' as const },
    lg: { root: 'p-0.75', icon: 'lg' as const },
  }
  ...
  return (
    <AriaButton
      ...
      className={cx(
        'flex cursor-pointer rounded-[3px] text-fg-quaternary outline-transparent transition duration-100 ease-linear hover:bg-primary-hover hover:text-fg-quaternary-hover focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring disabled:cursor-not-allowed',
      ...
      >
        {(state) => (
          <div className="flex flex-col gap-1.5">
            ...
            <SelectPopover size={'md'} triggerRef={placeholderRef} style={{ width: popoverWidth }} className={props?.popoverClassName}>
...
const InnerMultiSelect = ({ isDisabled, shortcut, shortcutClassName, placeholder }: Omit<MultiSelectProps, 'selectedItems' | 'children'>) => {
  ...
  const comboBoxContext = useContext(MultiSelectContext)
...
export const MultiSelectTagsValue = ({
  ...
  placeholder,
  shortcutClassName,
  placeholderIcon: PlaceholderIcon = SearchIcon,

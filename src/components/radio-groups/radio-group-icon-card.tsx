...
export type RadioGroupIconCardItemType = {
  value: string
  title: string
  description: string
...
export interface RadioGroupIconCardProps extends RadioGroupProps {
  size?: 'sm' | 'md'
  items: RadioGroupIconCardItemType[]
}
          ...
          {({ isDisabled, isSelected, isFocusVisible }) => (
            <>
              ...
              <div className="flex flex-col-reverse items-start justify-between gap-4 rounded-b-lg p-4 sm:flex-row sm:gap-1">
                ...
                {plan.badge && (
                  <Badge size="sm" type="badge-modern" color="success" dot>
                    {plan.badge}
                  </Badge>
                )}
              </div>

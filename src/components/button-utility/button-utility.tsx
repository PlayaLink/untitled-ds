...
export const styles = sortCx({
  ...
  icon: '*:data-icon:pointer-events-none *:data-icon:shrink-0 *:data-icon:text-current *:data-icon:transition-inherit-all',
})

export type ButtonUtilityColor = keyof typeof styles.colors
export type ButtonUtilitySize = keyof typeof styles.sizes
 ...
 */
export interface CommonButtonUtilityProps {
...
export interface ButtonUtilityButtonProps
  extends CommonButtonUtilityProps,
...
export const ButtonUtility = ({
  ...
  const content = (
    <Component
      aria-label={tooltip}
      {...props}
      className={cx(styles.base, styles.colors[color], styles.sizes[size], styles.icon, className)}
    >

...
export interface RadioGroupAvatarItemType {
  id: string
  name: string
  username: string
  title: string
...
export interface RadioGroupAvatarProps extends RadioGroupProps {
  size?: 'sm' | 'md'
  items: RadioGroupAvatarItemType[]
}
          ...
          {({ isDisabled, isSelected, isFocusVisible }) => (
            <>
              <div className={cx('flex flex-1', size === 'md' ? 'gap-3 md:gap-4' : 'gap-3')}>
                <Avatar alt={person.name} src={person.avatarUrl} size={size === 'md' ? 'md' : 'sm'} />

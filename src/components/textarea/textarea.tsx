...
/**
 * TextArea component
 * @docs https://www.untitledui.com/components/textarea
 */
...
export const styles = sortCx({
    base: "w-full scroll-py-3 rounded-lg bg-primary px-3.5 py-3 text-md text-primary shadow-xs ring-1 ring-border-primary transition duration-100 ease-linear ring-inset placeholder:text-placeholder autofill:rounded-lg autofill:text-primary focus:outline-hidden",
    resizeHandle: "[&::-webkit-resizer]:bg-(image:--resize-handle-bg) [&::-webkit-resizer]:bg-contain dark:[&::-webkit-resizer]:bg-(image:--resize-handle-bg-dark)",
    focused: "ring-2 ring-border-brand",
    disabled: "cursor-not-allowed bg-disabled-subtle text-disabled ring-border-disabled",
    invalid: "ring-border-error-subtle",
    invalidFocused: "ring-2 ring-border-error",
    wrapper: "group flex h-max w-full flex-col items-start justify-start gap-1.5",
});
        ...
        <AriaTextArea
            ...
            className={(state) =>
                cx(
                    styles.base,
                    styles.resizeHandle,
                    state.isFocused && !state.isDisabled && styles.focused,
                    state.isDisabled && styles.disabled,
                    state.isInvalid && styles.invalid,
                    state.isInvalid && state.isFocused && styles.invalidFocused,
